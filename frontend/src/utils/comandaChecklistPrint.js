...
const gerarCardComandaChecklist = ({
  nome = '',
  telefone = '',
  endereco = '',
  pagamento = '',
...
const COMANDA_CHECKLIST_PRINT_CSS = `
  ...
  body.multiplas-comandas-checklist {
    height: auto;
    min-height: 70mm;
  }

  body.multiplas-comandas-checklist .print-area {
    height: auto;
  }

  body.multiplas-comandas-checklist .comanda + .comanda {
...
const gerarDocumentoComandaChecklist = ({
  tituloJanela = 'Comanda',
  cardsHtml = '',
  multiplasComandas = false
} = {}) => `
  <html>
    <head>
      ...
      <style>
        ${COMANDA_PRINT_CSS}
        ${COMANDA_CHECKLIST_PRINT_CSS}
      </style>
    </head>
    <body class="${multiplasComandas ? 'multiplas-comandas-checklist' : ''}">
      <div class="print-area">${cardsHtml}</div>
    </body>
  </html>
`;

export const gerarHtmlComandaChecklist = (dados = {}) =>
  gerarDocumentoComandaChecklist({
    tituloJanela: dados?.tituloJanela || 'Comanda',
    cardsHtml: gerarCardComandaChecklist(dados),
    multiplasComandas: false
  });
...
export const gerarHtmlComandasChecklist = (lista = []) => {
  ...
  return gerarDocumentoComandaChecklist({
    tituloJanela: comandas[0]?.tituloJanela || `Comandas (${comandas.length})`,
    cardsHtml: comandas.map((dadosComanda) => gerarCardComandaChecklist(dadosComanda)).join(''),
    multiplasComandas: comandas.length > 1
  });
};
